...
class PatchedOpenAIClient(OpenAIClient):
    ...
    def enable_semantic_cache(
        self, enabled: bool = True, similarity_threshold: float = 0.9
    ) -> None:
...
async def run_benchmark(
    ...
    if hasattr(config, "use_semantic_cache") and config.use_semantic_cache:
        client.enable_semantic_cache(
            True, 0.85
        )  # Use 0.85 as default similarity threshold
    ...
    GLOBAL_MONITOR.enable(
        config.enable_monitoring if hasattr(config, "enable_monitoring") else True
    )
    ...
    try:
        ...
        if hasattr(config, "use_batched_requests") and config.use_batched_requests:
            print("  Batched requests enabled")

        if hasattr(config, "use_fast_similarity") and config.use_fast_similarity:
            print("  Fast similarity enabled")

        if (
            hasattr(config, "use_adaptive_temperature")
            and config.use_adaptive_temperature
        ):
            print(
                f"  Adaptive temperature enabled (initial: {config.initial_temperature}, min: {config.min_generation_temperature})"
            )
...
async def main():
    ...
    for scale in selected_scales:
        ...
        if len(scale_results) > 1:
            ...
            optimized = next(
                (r for r in scale_results if "All optimizations" in r["name"]), None
            )

            if baseline and optimized:
                speedup = (
                    baseline["elapsed"] / optimized["elapsed"]
