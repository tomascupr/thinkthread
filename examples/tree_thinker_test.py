...
async def test_with_dummy_client():
    ...
    async_result = await tree_thinker.solve_async(problem, beam_width=2, max_iterations=1)
    ...
    try:
        ...
        print(f"Error handling worked! Result status: {error_result.get('status', 'unknown')}")
    ...
    try:
        async_error_result = await error_thinker.solve_async(problem, beam_width=2, max_iterations=1)
        print(f"Async error handling worked! Result status: {async_error_result.get('status', 'unknown')}")
