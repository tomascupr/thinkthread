...
def example_sync_usage():
    ...
    llm_client = OpenAIClient(
        api_key=os.environ.get("OPENAI_API_KEY", ""), model_name="gpt-3.5-turbo"
    )
...
async def example_async_usage():
    ...
    llm_client = AnthropicClient(
        api_key=os.environ.get("ANTHROPIC_API_KEY", ""), model="claude-2"
    )
...
def example_custom_provider():
    ...
    result = tree_thinker.solve(problem=problem, beam_width=1, max_iterations=1)
