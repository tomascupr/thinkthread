...
@pytest.mark.parametrize(
    ...
    [
        ({"parallel_alternatives": False, "parallel_evaluation": False, "use_caching": False}, 1.0),
        ({"parallel_alternatives": True, "parallel_evaluation": False, "use_caching": False}, 1.8),
        ({"parallel_alternatives": False, "parallel_evaluation": True, "use_caching": False}, 1.5),
        ({"parallel_alternatives": True, "parallel_evaluation": True, "use_caching": False}, 2.5),
        ({"parallel_alternatives": False, "parallel_evaluation": False, "use_caching": True}, 1.5),
        ({"parallel_alternatives": True, "parallel_evaluation": True, "use_caching": True}, 3.0),
        ({"parallel_alternatives": True, "parallel_evaluation": True, "use_caching": True, 
          "early_termination": True, "early_termination_threshold": 0.9}, 3.5),
        ({"parallel_alternatives": True, "parallel_evaluation": True, "use_caching": True, 
          "use_batched_requests": True}, 3.5),
        ({"parallel_alternatives": True, "parallel_evaluation": True, "use_caching": True, 
          "use_fast_similarity": True}, 3.5),
        ({"parallel_alternatives": True, "parallel_evaluation": True, "use_caching": True, 
          "use_adaptive_temperature": True, "initial_temperature": 0.7, 
          "generation_temperature": 0.9, "min_generation_temperature": 0.5, 
          "temperature_decay_rate": 0.8}, 3.5),
        ({"parallel_alternatives": True, "parallel_evaluation": True, "use_caching": True, 
          "early_termination": True, "early_termination_threshold": 0.9, 
          "use_batched_requests": True, "use_fast_similarity": True, 
          "use_adaptive_temperature": True, "initial_temperature": 0.7, 
          "generation_temperature": 0.9, "min_generation_temperature": 0.5, 
          "temperature_decay_rate": 0.8}, 5.0),
    ],
)
@pytest.mark.asyncio
async def test_optimization_performance(optimization_config: Dict[str, Any], expected_speedup: float) -> None:
    ...
    if optimization_config.get("use_fast_similarity", False) and optimization_config.get("early_termination", False):
    ...
    if any(optimization_config.values()):
        ...
        assert actual_speedup >= min_expected_speedup, f"Expected at least {min_expected_speedup:.2f}x speedup, got {actual_speedup:.2f}x"
        
        if all(v for k, v in optimization_config.items() if k in [
            "parallel_alternatives", "parallel_evaluation", "use_caching", 
            "early_termination", "use_batched_requests", "use_fast_similarity",
            "use_adaptive_temperature"
        ]):
            assert actual_speedup >= 3.0, f"Expected at least 3x speedup with all optimizations, got {actual_speedup:.2f}x"
...
def test_fast_similarity() -> None:
    ...
    original_calculate_similarity = session._calculate_similarity
    original_calculate_fast_similarity = getattr(session, "_calculate_fast_similarity", None)
    ...
    print(f"Average similarity calculation time (fast): {sum(fast_similarity_times)/len(fast_similarity_times):.4f}s")
    print(f"Average similarity calculation time (standard): {sum(standard_similarity_times)/len(standard_similarity_times):.4f}s")
    print(f"Speedup: {standard_elapsed / fast_elapsed:.2f}x")
    
    assert sum(fast_similarity_times) <= sum(standard_similarity_times), "Expected fast similarity to be faster than standard similarity"
...
async def test_semantic_caching() -> None:
    ...
    start_time = time.time()
    response1 = await client.acomplete(prompts[0])
    first_request_time = time.time() - start_time
    
    start_time = time.time()
    response2 = await client.acomplete(prompts[1])
    semantic_hit_time = time.time() - start_time
    
    start_time = time.time()
    response3 = await client.acomplete(prompts[3])
    different_topic_time = time.time() - start_time
    
    start_time = time.time()
    response4 = await client.acomplete(prompts[0])
    ...
    assert semantic_hit_time < first_request_time, "Expected semantic cache hit to be faster than cache miss"
    assert semantic_hit_time > exact_hit_time, "Expected semantic cache hit to be slower than exact cache hit"
    assert different_topic_time > semantic_hit_time, "Expected different topic to be a cache miss"
    ...
    print(f"Semantically similar request (without semantic caching): {no_semantic_time:.4f}s")
    assert no_semantic_time > semantic_hit_time, "Expected request without semantic caching to be slower"
