...
def test_thinkthread_session_normal_case(mock_template_manager, mock_config):
    ...
    assert (
        mock_template_manager.render_template.call_count == 9
...
def test_evaluation_strategy_parse_evaluation(mock_template_manager, mock_config):
    ...
    assert (
        strategy._parse_evaluation(
            "After careful consideration, the best answer is Answer 2.", 3
        )
        == 1
    )
    ...
    assert (
        strategy._parse_evaluation(
            "Answer 1 is the best because it's more comprehensive.", 3
        )
        == 0
    )
    assert (
        strategy._parse_evaluation(
            "I would select Answer 3 as it provides more detail.", 3
        )
        == 2
    )
...
def test_thinkthread_session_custom_evaluation_strategy(mock_template_manager, mock_config):
    ...
    assert (
        mock_template_manager.render_template.call_count == 4
