...
def mock_template_manager():
    ...
    def render_side_effect(template_name, context):
        if template_name == "initial_prompt.j2":
            ...
            return f"Eval: {context['question']} Answers: {context['formatted_answers']}"
...
def test_self_evaluation():
    ...
    class TestEvaluator(ModelEvaluator):
        def evaluate(self, question, prev_answer, new_answer, llm_client, template_manager):
            return True
    ...
    assert client.call_count == 3  # Initial + 2 alternatives (no separate evaluation calls)
